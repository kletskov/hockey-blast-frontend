...
def create_app(db_name):
    ...
    def index():
        try:
            ...
            all_time_skater_points = db.session.query(OrgStatsSkater, Human, Organization).join(Human, OrgStatsSkater.human_id == Human.id).join(Organization, OrgStatsSkater.org_id == Organization.id).filter(OrgStatsSkater.points > 0, OrgStatsSkater.org_id != ALL_ORGS_ID).order_by(OrgStatsSkater.points.desc()).limit(top_n).all()
            all_time_goalie_games_played = db.session.query(OrgStatsGoalie, Human, Organization).join(Human, OrgStatsGoalie.human_id == Human.id).join(Organization, OrgStatsGoalie.org_id == Organization.id).filter(OrgStatsGoalie.games_played > 0, OrgStatsGoalie.org_id != ALL_ORGS_ID).order_by(OrgStatsGoalie.games_played.desc(), OrgStatsGoalie.save_percentage.desc()).limit(top_n).all()
            all_time_referee_games_reffed = db.session.query(OrgStatsReferee, Human, Organization).join(Human, OrgStatsReferee.human_id == Human.id).join(Organization, OrgStatsReferee.org_id == Organization.id).filter(OrgStatsReferee.games_reffed > 0, OrgStatsReferee.org_id != ALL_ORGS_ID).order_by(OrgStatsReferee.games_reffed.desc(), (OrgStatsReferee.gm_given + OrgStatsReferee.penalties_given).desc()).limit(top_n).all()
            all_time_scorekeeper_games = db.session.query(OrgStatsHuman, Human, Organization).join(Human, OrgStatsHuman.human_id == Human.id).join(Organization, OrgStatsHuman.org_id == Organization.id).filter(OrgStatsHuman.games_scorekeeper > 0, OrgStatsHuman.human_id != fake_human_id, OrgStatsHuman.org_id != ALL_ORGS_ID).order_by(OrgStatsHuman.games_scorekeeper.desc()).limit(top_n).all()
