"""add org to TeamInTTS

Revision ID: 8c54d9f4da79
Revises: dddefad7b6d2
Create Date: 2024-12-11 12:27:38.613642

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '8c54d9f4da79'
down_revision = 'dddefad7b6d2'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('teams_in_tts', schema=None) as batch_op:
        batch_op.add_column(sa.Column('org_id', sa.Integer(), nullable=True))
        batch_op.drop_constraint('_team_tts_uc', type_='unique')
        batch_op.create_unique_constraint('_org_team_tts_uc', ['org_id', 'team_id', 'tts_team_id'])
        batch_op.create_foreign_key(None, 'organizations', ['org_id'], ['id'])

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('teams_in_tts', schema=None) as batch_op:
        batch_op.drop_constraint(None, type_='foreignkey')
        batch_op.drop_constraint('_org_team_tts_uc', type_='unique')
        batch_op.create_unique_constraint('_team_tts_uc', ['team_id', 'tts_team_id'])
        batch_op.drop_column('org_id')

    # ### end Alembic commands ###
