...
from hockey_blast_common_lib.models import (Division, Game, Human,
                                            ...
                                            db)
from hockey_blast_common_lib.stats_models import (DivisionStatsGoalie,
                                                  LevelStatsGoalie,
                                                  OrgStatsGoalie)
from hockey_blast_common_lib.stats_utils import ALL_ORGS_ID

from .goalie_performance_dropdowns import (filter_levels, filter_seasons,
                                           filter_teams,
                                           get_divisions_and_seasons,
                                           get_levels_for_goalie_in_org)
from .team_division_goalie_stats import (compute_goalie_stats,
                                         team_division_goalie_stats_bp)

goalie_performance_bp = Blueprint("goalie_performance", __name__)
goalie_performance_bp.register_blueprint(
    team_division_goalie_stats_bp, url_prefix="/team_division_goalie_stats"
)
...
def append_goalie_performance_result(
    goalie_performance_results, stats, context, context_value=0
):
    ...
    goalie_performance_results.append(
        {
            ...
            "goals_allowed_rank": format_rank_percentile(
                goals_allowed_rank,
                (
                    stats["total_in_rank"]
                    if isinstance(stats, dict)
            ...
            "goals_allowed_per_game_rank": format_rank_percentile(
                goals_allowed_per_game_rank,
                (
                    stats["total_in_rank"]
                    if isinstance(stats, dict)
                    else stats.total_in_rank
            ...
            "shots_faced_rank": format_rank_percentile(
                shots_faced_rank,
                (
                    stats["total_in_rank"]
                    if isinstance(stats, dict)
            ...
            "save_percentage_rank": format_rank_percentile(
                save_percentage_rank,
                (
                    stats["total_in_rank"]
                    if isinstance(stats, dict)
                    else stats.total_in_rank
...
def goalie_performance():
    ...
    if human_id:
        ...
        organizations = (
            ...
            .join(OrgStatsGoalie, Organization.id == OrgStatsGoalie.org_id)
            .filter(OrgStatsGoalie.human_id == human_id)
            .all()
        )
    else:
        ...
        if level_id is None:
            if human_id:
                ...
                for level in levels:
                    ...
                    level_stats = (
                        query.order_by(LevelStatsGoalie.goals_allowed_per_game_rank)
                        .limit(top_n)
                        .all()
                    )
            ...
            if season_id is None:
                if human_id:
                    ...
                    for division in divisions:
                        ...
                        division_stats = (
                            query.order_by(
                                DivisionStatsGoalie.goals_allowed_per_game_rank
                            )
                            .limit(top_n)
                            .all()
                        )
                ...
                if team_id is None:
                    ...
                    all_goalies_stats = (
                        db.session.query(DivisionStatsGoalie)
                        .filter(DivisionStatsGoalie.division_id == division_id)
                        .order_by(DivisionStatsGoalie.goals_allowed_per_game_rank)
                        .all()
                    )
                    ...
                    for stats in all_goalies_stats:
                        ...
                        if player:
                            ...
                            append_goalie_performance_result(
                                all_goalies_results, stats, link
                            )
                else:
