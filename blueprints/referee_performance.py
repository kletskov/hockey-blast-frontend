...
from hockey_blast_common_lib.models import (Division, Game, Human, Level,
...
from .referee_performance_dropdowns import (filter_levels, filter_seasons,
                                            filter_teams)
...
def append_referee_performance_result(
    referee_performance_results, stats, context, context_value=0
):
    ...
    referee_performance_results.append(
        {
            ...
            "games_participated_rank": format_rank_percentile(
                games_participated_rank, total_in_rank
            ),
            "penalties_given": penalties_given,
            "penalties_given_rank": format_rank_percentile(
                penalties_given_rank, total_in_rank
            ),
            "penalties_per_game": f"{penalties_per_game:.2f}",
            "penalties_per_game_rank": format_rank_percentile(
                penalties_per_game_rank, total_in_rank
            ),
            "gm_given": gm_given,
...
def referee_performance():
    ...
    if human_id:
        ...
        organizations = (
            ...
            .join(OrgStatsReferee, Organization.id == OrgStatsReferee.org_id)
            .filter(OrgStatsReferee.human_id == human_id)
            .all()
        )
    else:
    ...
    if org_id == ALL_ORGS_ID:
        if human_id:
            ...
            for stats in org_stats:
                ...
                append_referee_performance_result(
                    referee_performance_results, stats, context
                )

            # Apply sorting, then min_games filter, then limit
            referee_performance_results.sort(
                key=lambda x: (x["games_participated"]), reverse=True
            )
            referee_performance_results = [
                r
                for r in referee_performance_results
                if r["games_participated"] >= min_games
            ]
        ...
        org_name = (
            organization.organization_name if organization else "Selected Organization"
        )
        ...
        if level_id is None:
            ...
            for stats in org_stats:
                human = (
                    db.session.query(Human).filter(Human.id == stats.human_id).first()
                )
                if human:
                    ...
                    if human_id and human_id == stats.human_id:
                        append_referee_performance_result(
                            referee_performance_results, stats, org_name
                        )

            # Apply proper sorting, then limit the results
            all_referees_results.sort(
                key=lambda x: (x["games_participated"]), reverse=True
            )
            ...
            if human_id:
                ...
                level_stats = query.order_by(
                    LevelStatsReferee.penalties_per_game_rank
                ).all()
            ...
            level_stats = (
                db.session.query(LevelStatsReferee)
                .filter(
                    LevelStatsReferee.level_id == level_id,
                    LevelStatsReferee.games_participated >= min_games,
                )
                .order_by(LevelStatsReferee.penalties_per_game_rank)
                .all()
            )
            ...
            if division:
                ...
                if human_id:
                    human_stats = (
                        db.session.query(DivisionStatsReferee)
                        .filter(
                    ...
                    if human_stats:
                        ...
                        append_referee_performance_result(
                            referee_performance_results, human_stats, context
                        )
                ...
                all_referees_stats = (
                    ...
                    .order_by(DivisionStatsReferee.penalties_per_game_rank)
                    .all()
                )
                ...
                for stats in all_referees_stats:
                    ...
                    if player:
                        ...
                        append_referee_performance_result(
                            all_referees_results, stats, link
                        )
