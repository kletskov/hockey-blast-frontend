...
from hockey_blast_common_lib.stats_models import DivisionStatsReferee, LevelStatsReferee

def get_levels_for_referee_in_org(org_id, human_id=None):
    try:
        org_id = int(org_id)
    except (ValueError, TypeError):
        return []

    query = db.session.query(Level).join(LevelStatsReferee, Level.id == LevelStatsReferee.level_id).filter(
        Level.org_id == org_id
    )
...
def get_divisions_and_seasons(org_id, level_id, human_id=None):
    ...
    query = db.session.query(Division).join(DivisionStatsReferee, Division.id == DivisionStatsReferee.division_id).filter(
        Division.org_id == org_id,
        Division.level_id == level_id
    )

    if human_id:
        query = query.filter(DivisionStatsReferee.human_id == human_id)
...
def filter_levels(org_id, human_id):
    levels = get_levels_for_referee_in_org(org_id, human_id)
...
def filter_teams(org_id, level_id, season_id, human_id=None):
    ...
    query = db.session.query(GameRoster).join(Game, GameRoster.game_id == Game.id).filter(
        ...
        GameRoster.role.ilike('r')  # Role 'r' for referee
    )
