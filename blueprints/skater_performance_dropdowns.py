...
from hockey_blast_common_lib.models import (Division, Game, GameRoster, Level,
                                            Season, Team, db)
from hockey_blast_common_lib.stats_models import (DivisionStatsSkater,
                                                  LevelStatsSkater)


def get_levels_for_skater_in_org(org_id, human_id=None):
    try:
        org_id = int(org_id)
    ...
    query = (
        ...
        .join(LevelStatsSkater, Level.id == LevelStatsSkater.level_id)
        .filter(Level.org_id == org_id)
    )

    if human_id:
        query = query.filter(LevelStatsSkater.human_id == human_id)

    levels = query.order_by(Level.level_name).all()
...
def get_divisions_and_seasons(org_id, level_id, human_id=None):
    ...
    query = (
        ...
        .join(DivisionStatsSkater, Division.id == DivisionStatsSkater.division_id)
        .filter(Division.org_id == org_id, Division.level_id == level_id)
    )

    if human_id:
        query = query.filter(DivisionStatsSkater.human_id == human_id)
...
def filter_teams(org_id, level_id, season_id, human_id=None):
    ...
    query = (
        ...
        .filter(Game.division_id == division.id, ~GameRoster.role.ilike("g"))
    )
