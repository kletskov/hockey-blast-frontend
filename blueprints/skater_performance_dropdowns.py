...
def get_levels_for_skater_in_org(org_id, human_id=None):
    ...
    query = db.session.query(Level).join(LevelStatsSkater, Level.id == LevelStatsSkater.level_id).filter(
        Level.org_id == org_id
    )
...
def get_divisions_and_seasons(org_id, level_id, human_id=None):
    ...
    query = db.session.query(Division).join(DivisionStatsSkater, Division.id == DivisionStatsSkater.division_id).filter(
