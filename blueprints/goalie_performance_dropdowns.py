...
def get_levels_for_goalie_in_org(org_id, human_id):
    try:
        org_id = int(org_id)
    except (ValueError, TypeError):
        return []

    levels = db.session.query(Level).join(LevelStatsGoalie, Level.id == LevelStatsGoalie.level_id).filter(
...
def get_divisions_and_seasons(org_id, level_id, human_id):
    ...
    divisions = db.session.query(Division).join(DivisionStatsGoalie, Division.id == DivisionStatsGoalie.division_id).filter(
