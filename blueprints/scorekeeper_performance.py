...
from hockey_blast_common_lib.models import (Game, Human, Level,
                                            Organization,
                                            ScorekeeperSaveQuality, Season,
                                            db)
from hockey_blast_common_lib.stats_models import OrgStatsScorekeeper
from hockey_blast_common_lib.stats_utils import ALL_ORGS_ID

from .scorekeeper_performance_dropdowns import (
    filter_levels, filter_seasons, filter_teams)
...
def append_scorekeeper_performance_result(
    scorekeeper_performance_results, stats, context, context_value=0, quality_data=None
):
    ...
    scorekeeper_performance_results.append(
        {
            ...
            "sog_per_game_rank": format_rank_percentile(
                sog_per_game_rank, total_in_rank
            ),
...
def get_scorekeeper_quality_data(human_id):
    ...
    try:
        quality_records = (
            db.session.query(ScorekeeperSaveQuality)
            .filter(ScorekeeperSaveQuality.scorekeeper_id == human_id)
            .all()
        )
        ...
        total_saves_20sec = sum(
            record.max_saves_per_20sec for record in quality_records
        )
        ...
        return {
            ...
            "avg_saves_20sec": (
                total_saves_20sec / total_games if total_games > 0 else 0
            ),
            "max_saves_5sec": max_saves_5sec,
            "max_saves_20sec": max_saves_20sec,
...
def scorekeeper_performance():
    ...
    if human_id:
        ...
        organizations = (
            ...
            .join(OrgStatsScorekeeper, Organization.id == OrgStatsScorekeeper.org_id)
            .filter(OrgStatsScorekeeper.human_id == human_id)
            .all()
        )
    else:
    ...
    if org_id == ALL_ORGS_ID:
        if human_id:
            ...
            for stats in org_stats:
                ...
                append_scorekeeper_performance_result(
                    scorekeeper_performance_results,
                    stats,
                    context,
            ...
            scorekeeper_performance_results.sort(
                key=lambda x: (x["games_participated"]), reverse=True
            )
            scorekeeper_performance_results = [
                r
                for r in scorekeeper_performance_results
        ...
        if level_id is None:
            ...
            for stats in org_stats:
                ...
                if human:
                    ...
                    quality_data = get_scorekeeper_quality_data(stats.human_id)
                    append_scorekeeper_performance_result(
                        all_scorekeepers_results, stats, link, quality_data=quality_data
                    )
