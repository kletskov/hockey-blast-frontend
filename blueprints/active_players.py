...
from hockey_blast_common_lib.models import (HumansInLevels, LevelsMonthly,
                                            db)
...
def get_plot_data():
    ...
    print(
        f"Debug: X Axis: {x_axis}, Plot 1: {plot_1}, Plot 2: {plot_2}, Min Games: {min_games}"
    )
    ...
    def get_query(level):
        subquery = (
            ...
            .group_by(HumansInLevels.human_id)
            .subquery()
        )
        ...
        if x_axis == "year_month" or x_axis == "year_over_year":
            query = (
                db.session.query(
                    LevelsMonthly.year,
                    LevelsMonthly.month,
                    db.func.count(HumansInLevels.human_id.distinct()).label(
                        "active_players"
                    ),
                )
                .join(
                    HumansInLevels, LevelsMonthly.id == HumansInLevels.levels_monthly_id
                )
                .join(subquery, subquery.c.human_id == HumansInLevels.human_id)
                .filter(subquery.c.games_played >= min_games)
                .group_by(LevelsMonthly.year, LevelsMonthly.month)
            )
            if level != "all":
            ...
            query = (
                ...
                .join(subquery, subquery.c.human_id == HumansInLevels.human_id)
                .filter(subquery.c.games_played >= min_games)
                .group_by(LevelsMonthly.season_number, LevelsMonthly.season_name)
            )
            if level != "all":
                query = query.filter(LevelsMonthly.level == level)
        return query
    ...
    if x_axis == "year_month":
        df_1["x"] = (
            df_1["year"].astype(str) + "-" + df_1["month"].astype(str).str.zfill(2)
        )
        df_2["x"] = (
            df_2["year"].astype(str) + "-" + df_2["month"].astype(str).str.zfill(2)
        )
        ...
        data_1 = {
            str(year): df_1[df_1["year"] == year]["active_players"].tolist()
            for year in years_1
        }
        return jsonify({"months": months, "years": data_1, "plot_1_name": plot_1})
    else:
