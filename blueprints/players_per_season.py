...
def players_per_season():
    ...
    items = [SeasonItem(result.season_number, result.season_name, result.start_date, result.end_date, result.num_players) for result in results]
    ...
    for i, txt in enumerate(num_players):
        plt.annotate(txt, (season_names[i], num_players[i]), textcoords="offset points", xytext=(0,10), ha='center')
    ...
    return render_template('players_per_season.html', players_per_season=items, plot_url=plot_url)
