...
from hockey_blast_common_lib.models import db, Game, Division, Team, Season
import pandas as pd
...
def get_time_of_games_data():
    ...
    print(f"Debug: Plot Type: {plot_type}, Level 1: {level_1}, Level 2: {level_2}, Plot Level: {plot_level}")
    def get_query(level, league_number=27):
        ...
        ).join(Division, Game.division_id == Division.id).filter(Game.game_type != 'Practice').filter(Division.league_number == league_number)
    ...
    if plot_type == 'compare_levels_by_season':
        ...
        if df_1.empty and df_2.empty:
            return jsonify({'x': [], 'y_avg_1': [], 'y_avg_2': [], 'y_last_slot_1': [], 'y_last_slot_2': []})

        if df_1.empty:
            df_1 = pd.DataFrame(columns=['season_number', 'time_minutes', 'last_slot_game'])
        if df_2.empty:
            df_2 = pd.DataFrame(columns=['season_number', 'time_minutes', 'last_slot_game'])
        ...
        avg_start_time_1 = df_1.groupby(['season_number'])['time_minutes'].mean().round()
        avg_start_time_2 = df_2.groupby(['season_number'])['time_minutes'].mean().round()

        last_slot_percentage_1 = df_1.groupby(['season_number'])['last_slot_game'].mean() * 100
        last_slot_percentage_2 = df_2.groupby(['season_number'])['last_slot_game'].mean() * 100

        # Ensure unique X-axis values
        common_index = avg_start_time_1.index.intersection(avg_start_time_2.index)
        x_values = [f"{season_number} - {season_map[season_number]}" for season_number in common_index]
        ...
        print("Distinct league_number values in the DataFrame:", distinct_league_numbers)
        ...
        df_visitor = df[['time_minutes', 'last_slot_game', 'visitor_team_id', 'season_number', 'level']].rename(columns={'visitor_team_id': 'team_id'})
        df_home = df[['time_minutes', 'last_slot_game', 'home_team_id', 'season_number', 'level']].rename(columns={'home_team_id': 'team_id'})
        df = pd.concat([df_visitor, df_home])
        
        min_games_per_group = 2  # Minimum number of games to be included in the average
        avg_start_time = df.groupby(['season_number', 'team_id'])['time_minutes'].agg(['mean', 'count'])
        avg_start_time = avg_start_time[avg_start_time['count'] > min_games_per_group]['mean'].round()
        last_slot_percentage = df.groupby(['season_number', 'team_id'])['last_slot_game'].agg(['mean', 'count'])
        last_slot_percentage = last_slot_percentage[last_slot_percentage['count'] > min_games_per_group]['mean'] * 100

        # Ensure unique X-axis values
        x_values = [f"{season_number} - {season_map[season_number]}" for season_number in avg_start_time.index.get_level_values('season_number').unique()]

        teams = df['team_id'].unique()  # Get unique teams from the dataframe with the specified level
        teams = [team for team in teams if team_map.get(team)]  # Filter out teams that map to empty name
        traces_avg = []
        traces_last_slot = []

        for team in teams:
            if team in avg_start_time.index.get_level_values('team_id'):
                team_avg_start_time = avg_start_time.xs(team, level='team_id').reindex(avg_start_time.index.get_level_values('season_number').unique(), fill_value=None)
                team_last_slot_percentage = last_slot_percentage.xs(team, level='team_id').reindex(last_slot_percentage.index.get_level_values('season_number').unique(), fill_value=None)
                ...
                traces_last_slot.append({
                    'x': x_values,
                    'y': [None if pd.isna(y) else y for y in team_last_slot_percentage],
                    'mode': 'lines+markers',
                    'name': team_name
                })
