...
from hockey_blast_common_lib.models import (Game, GameRoster, db)
...
def compute_goalie_stats(games, team_id):
    ...
    goalie_stats = (
        db.session.query(
            ...
            func.sum(
                case(
                    (GameRoster.team_id == Game.home_team_id, Game.visitor_final_score),
                    else_=Game.home_final_score,
                )
            ).label("goals_allowed"),
            func.sum(
                case(
                    (
                        ...
                        Game.visitor_period_1_shots
                        + Game.visitor_period_2_shots
                        + Game.visitor_period_3_shots
                        + Game.visitor_ot_shots
                        + Game.visitor_so_shots,
                    ),
                    else_=Game.home_period_1_shots
                    + Game.home_period_2_shots
                    + Game.home_period_3_shots
                    + Game.home_ot_shots
                    + Game.home_so_shots,
                )
            ).label("shots_faced"),
    ...
    for key, stat in stats_dict.items():
        if stat["games_participated"] > 0:
            stat["goals_allowed_per_game"] = (
                stat["goals_allowed"] / stat["games_participated"]
            )
            stat["save_percentage"] = (
                1 - (stat["goals_allowed"] / stat["shots_faced"])
    ...
    def assign_ranks(stats_dict, field, reverse=True):
        sorted_stats = sorted(
            stats_dict.items(), key=lambda x: x[1][field], reverse=reverse
        )
