...
from hockey_blast_common_lib.models import (Game, GameRoster, Goal, Penalty, db)
...
def compute_skater_stats(games, team_id):
    ...
    for human_id in human_ids_to_filter:
        if human_id in stats_dict:
            del stats_dict[human_id]
    ...
    for key, stat in stats_dict.items():
        if stat["games_participated"] > 0:
            ...
            stat["gm_penalties_per_game"] = (
                stat["gm_penalties"] / stat["games_participated"]
            )
...
def team_division_skater_stats():
    ...
    return jsonify({"all_stats": stats_dict})
