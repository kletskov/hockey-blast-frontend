...
def team_stats():
    ...
    games = (
        ...
        .filter((Game.home_team_id == team_id) | (Game.visitor_team_id == team_id))
        .all()
    )
    ...
    last_division_name = (
        db.session.query(Division).filter(Division.id == division_id).first().level
        if division_id
        else None
    )
    ...
    # Existing logic
    goals_scored = (
        db.session.query(Goal).filter(Goal.scoring_team_id == team_id).count()
    )
    goals_against = (
        db.session.query(Goal).filter(Goal.opposing_team_id == team_id).count()
    )
    total_goals = goals_scored + goals_against
    goals_scored_percentage = (
        (goals_scored / total_goals * 100) if total_goals > 0 else 0
    )
    goals_against_percentage = (
        (goals_against / total_goals * 100) if total_goals > 0 else 0
    )
    ...
    for game in completed_games:
        if (
            ...
            and game.visitor_final_score > game.home_final_score
        ):
            total_wins += 1
        elif (
            game.home_team_id == team_id
    ...
    win_percentage = (
        (total_wins / total_games_played * 100) if total_games_played > 0 else 0
    )
    loss_percentage = (
        (total_losses / total_games_played * 100) if total_games_played > 0 else 0
    )

    playoff_and_championship_games = [
        game
        for game in completed_games
    ...
    playoff_win_percentage = (
        (playoff_wins / playoff_games_played * 100) if playoff_games_played > 0 else 0
    )
    playoff_loss_percentage = (
        (playoff_losses / playoff_games_played * 100) if playoff_games_played > 0 else 0
    )

    championship_games = [
        game for game in completed_games if game.game_type == "Championship"
    ]
    ...
    championship_win_percentage = (
        (championship_wins / championship_games_played * 100)
        if championship_games_played > 0
        else 0
    )
    championship_loss_percentage = (
        (championship_losses / championship_games_played * 100)
        if championship_games_played > 0
        else 0
    )
    ...
    playoff_and_championship_goals_against = (
        db.session.query(Goal)
        .join(Game)
        .filter(
    ...
    total_playoff_goals = (
        playoff_and_championship_goals_scored + playoff_and_championship_goals_against
    )
    playoff_goals_scored_percentage = (
        (playoff_and_championship_goals_scored / total_playoff_goals * 100)
        if total_playoff_goals > 0
        else 0
    )
    playoff_goals_against_percentage = (
        (playoff_and_championship_goals_against / total_playoff_goals * 100)
        if total_playoff_goals > 0
        else 0
    )
    ...
    goals_by_team = (
        ...
        .filter(Goal.scoring_team_id == team_id)
        .group_by(Goal.opposing_team_id)
        .all()
    )
    ...
    goals_against_team = (
        ...
        .filter(Goal.opposing_team_id == team_id)
        .group_by(Goal.scoring_team_id)
        .all()
    )
    ...
    for g in goals_by_team:
        if g.opponent_team_id not in performance_stats:
            performance_stats[g.opponent_team_id] = {
                "goals_scored": 0,
                "goals_against": 0,
            }
    ...
    for opponent_team_id, stats in performance_stats.items():
        if opponent_team_id == team_id:
            continue
        opponent_team = (
            db.session.query(Team).filter(Team.id == opponent_team_id).first()
        )
        if opponent_team:
    ...
    human_game_counts = (
        ...
        .filter(GameRoster.team_id == team_id)
        .group_by(GameRoster.human_id)
        .order_by(db.func.count(GameRoster.game_id).desc())
        .limit(top_n)
        .all()
    )
