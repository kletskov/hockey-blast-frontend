...
def filter_games():
    ...
    if game_status == "completed":
        query = query.filter(Game.status.ilike("Final%")).order_by(
            Game.date.desc(), Game.time.desc()
        )
    elif game_status == "scheduled":
        today = date.today()
        query = query.filter(~Game.status.ilike("Final%"), Game.date >= today).order_by(
            Game.date.asc(), Game.time.asc()
        )
    elif game_status == "all":
        query = query.order_by(Game.date.desc(), Game.time.desc())

    if level_id:
        query = query.join(Division, Game.division_id == Division.id).filter(
    ...
    return jsonify({"games": games_data, "team_stats": team_stats})
