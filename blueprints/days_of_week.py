...
from hockey_blast_common_lib.models import db, Game, Division, Organization, Level, Season
...
def index():
    ...
    all_orgs_option = Organization(id=ALL_ORGS_ID, organization_name="All Organizations")
    ...
    return render_template('days_of_week.html',
...
def filter_days():
    try:
        ...
        # Robust parsing
        try:
            org_id = int(org_id) if org_id and org_id != 'null' and org_id != '' else None
            level_id = int(level_id) if level_id and level_id != 'null' and level_id != '' else None
            season_id = int(season_id) if season_id and season_id != 'null' and season_id != '' else None
        ...
        chart_order_days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]

        if level_id is not None:
            # Break down by seasons
            rows = []
            season_query = db.session.query(Season).filter(Season.org_id == org_id).order_by(Season.season_number.desc())
            ...
            for season in seasons:
                ...
                query = query.filter(Division.level_id == level_id, Division.season_id == season.id)
                ...
                if any(season_row[day] > 0 for day in chart_order_days):
                    rows.append({"label": season.season_name, "data": [season_row[day] for day in chart_order_days]})

            return jsonify({
                "header": chart_order_days,
                "rows": rows
            })
        else:
            ...
            for level in levels:
                ...
                rows.append({"label": level_name, "data": [row[day] for day in chart_order_days]})

            # Add total games row at the top
            rows.insert(0, {"label": "Total Games", "data": [total_games_row[day] for day in chart_order_days]})
