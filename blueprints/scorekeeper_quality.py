...
from hockey_blast_common_lib.models import Game, Human, db
...
def scorekeeper_quality():
    ...
    return render_template("scorekeeper_quality.html", top_n=top_n, min_games=min_games)
...
def filter_scorekeeper_quality():
    ...
    scorekeepers_data = get_scorekeepers_with_quality_issues(
        min_games=min_games, limit=top_n
    )
    ...
    for scorekeeper in scorekeepers_data:
        ...
        if scorekeeper["first_game_id"]:
            first_game = (
                ...
                .filter(Game.id == scorekeeper["first_game_id"])
                .first()
            )
        if scorekeeper["last_game_id"]:
            last_game = (
                db.session.query(Game.date)
                .filter(Game.id == scorekeeper["last_game_id"])
                .first()
            )
        ...
        results.append(
            {
                ...
                "quality_rank": format_rank_percentile(
                    scorekeeper["quality_rank"],
                    scorekeeper["total_in_rank"],
                    reverse=True,
                ),
                ...
                "first_game": (
                    format_date_link(first_game.date, scorekeeper["first_game_id"])
                    if first_game
                    else None
                ),
                "last_game": (
                    format_date_link(last_game.date, scorekeeper["last_game_id"])
                    if last_game
                    else None
                ),
            }
    ...
    return jsonify(
        {"scorekeepers": results, "org_name": org_name, "total_found": len(results)}
    )
