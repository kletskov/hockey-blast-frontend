...
from hockey_blast_common_lib.models import (Division, Game, League, Season,
                                            db)
...
def interactive_plot():
    divisions = (
        db.session.query(Division.level).filter(Division.org_id == 1).distinct().all()
    )
    divisions = sorted(
        [division[0] for division in divisions]
...
def get_game_shootout_data():
    ...
    query = (
        ...
        .join(
            Season,
            and_(
                Division.season_number == Season.season_number,
                Division.league_number == Season.league_number,
            ),
        )
    )
    ...
    query = query.filter(
        Game.home_final_score.isnot(None), Game.visitor_final_score.isnot(None)
    )
    ...
    df = pd.DataFrame(
        [
            (
                ...
                g.season_number,
                g.home_final_score,
                g.visitor_final_score,
                g.home_period_1_score,
                g.home_period_2_score,
                g.home_period_3_score,
                g.visitor_period_1_score,
                g.visitor_period_2_score,
                g.visitor_period_3_score,
                g.visitor_so_shots,
                g.home_so_shots,
                g.game_type,
            )
        ...
        columns=[
            ...
            "home_period_2_score",
            "home_period_3_score",
            "visitor_period_1_score",
            "visitor_period_2_score",
            "visitor_period_3_score",
            "visitor_so_shots",
            "home_so_shots",
            "game_type",
        ],
    ...
    df["regulation_home_score"] = (
        df["home_period_1_score"]
        + df["home_period_2_score"]
        + df["home_period_3_score"]
    )
    df["regulation_visitor_score"] = (
        df["visitor_period_1_score"]
        + df["visitor_period_2_score"]
        + df["visitor_period_3_score"]
    )
    df["is_tie"] = df["regulation_home_score"] == df["regulation_visitor_score"]
    df["so_shots_present"] = (df["visitor_so_shots"] + df["home_so_shots"]) > 0

    df["went_to_so"] = (
        (df["regulation_home_score"] != df["home_final_score"])
        | (df["regulation_visitor_score"] != df["visitor_final_score"])
        | (df["so_shots_present"])
    )
    ...
    shootout_percentage = (shootout_games / tie_games).replace(
        [float("inf"), -float("inf")], 0
    ).fillna(0) * 100

    # Get season names for the selected league_number
    season_names = (
        db.session.query(Season.season_number, Season.season_name)
        .filter(Season.league_number == league_number)
        .all()
    )
    season_name_dict = {
        season.season_number: season.season_name for season in season_names
    }
    ...
    if x_axis == "year_month":
        ...
        x_display_names = [
            season_name_dict[season_number] for season_number in x_names_for_display
        ]
