...
from hockey_blast_common_lib.models import Human, db
from hockey_blast_common_lib.stats_models import (OrgStatsGoalie,
                                                  OrgStatsHuman,
                                                  OrgStatsReferee,
                                                  OrgStatsScorekeeper,
                                                  OrgStatsSkater)
...
def get_top_skaters_data(session, org_id, category, top_n_stats=10):
    ...
    top_players = (
        session.query(OrgStatsSkater, Human)
        .join(Human, Human.id == OrgStatsSkater.human_id)
        .filter(
            OrgStatsSkater.org_id == org_id,
            OrgStatsSkater.human_id != fake_human_id,
            OrgStatsSkater.games_participated >= 50,
        )
        .order_by(getattr(OrgStatsSkater, category).desc())
        .limit(top_n_stats)
        .all()
    )
...
def get_top_goalies_data(session, org_id, category, top_n_stats=10):
    fake_human_id = get_fake_human_for_stats(session)
    top_players = (
        session.query(OrgStatsGoalie, Human)
        .join(Human, Human.id == OrgStatsGoalie.human_id)
        .filter(
        ...
        .order_by(getattr(OrgStatsGoalie, category).desc())
        .limit(top_n_stats)
        .all()
    )
...
def get_top_referees_data(session, org_id, category, top_n_stats=10):
    fake_human_id = get_fake_human_for_stats(session)
    top_players = (
        session.query(OrgStatsReferee, Human)
        .join(Human, Human.id == OrgStatsReferee.human_id)
        .filter(
        ...
        .order_by(getattr(OrgStatsReferee, category).desc())
        .limit(top_n_stats)
        .all()
    )
...
def get_top_scorekeepers_data(session, org_id, category, top_n_stats=10):
    fake_human_id = get_fake_human_for_stats(session)
    top_players = (
        session.query(OrgStatsScorekeeper, Human)
        .join(Human, Human.id == OrgStatsScorekeeper.human_id)
        .filter(
            OrgStatsScorekeeper.org_id == org_id,
            OrgStatsScorekeeper.human_id != fake_human_id,
            OrgStatsScorekeeper.games_participated >= 50,
        )
        .order_by(getattr(OrgStatsScorekeeper, category).desc())
        .limit(top_n_stats)
        .all()
    )
...
def get_top_humans_data(session, org_id, top_n_stats=10):
    fake_human_id = get_fake_human_for_stats(session)
    top_humans = (
        session.query(OrgStatsHuman, Human)
        .join(Human, Human.id == OrgStatsHuman.human_id)
        .filter(
        ...
        .order_by(OrgStatsHuman.games_total.desc())
        .limit(top_n_stats)
        .all()
    )
...
def hall_of_fame():
    ...
    top_games_played_data = get_top_skaters_data(
        db.session, org_id, "games_participated", top_n_stats
    )
    ...
    top_points_per_game_data = get_top_skaters_data(
        db.session, org_id, "points_per_game", top_n_stats
    )
    top_penalties_per_game_data = get_top_skaters_data(
        db.session, org_id, "penalties_per_game", top_n_stats
    )

    # Fetch top goalies in different categories
    top_goalies_data = get_top_goalies_data(
        db.session, org_id, "games_participated", top_n_stats
    )
    top_goalies_save_percentage_data = get_top_goalies_data(
        db.session, org_id, "save_percentage", top_n_stats
    )

    # Fetch top referees in different categories
    top_referees_data = get_top_referees_data(
        db.session, org_id, "games_participated", top_n_stats
    )

    # Fetch top scorekeepers in different categories
    top_scorekeepers_data = get_top_scorekeepers_data(
        db.session, org_id, "games_participated", top_n_stats
    )
    ...
    return render_template(
        "hall_of_fame.html",
        top_humans=humans_data,
        top_games_played=top_games_played_data,
        top_goals=top_goals_data,
        top_points=top_points_data,
        top_points_per_game=top_points_per_game_data,
        top_penalties_per_game=top_penalties_per_game_data,
        top_goalies=top_goalies_data,
        top_goalies_save_percentage=top_goalies_save_percentage_data,
        top_referees=top_referees_data,
        top_scorekeepers=top_scorekeepers_data,
    )
