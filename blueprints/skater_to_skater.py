...
from hockey_blast_common_lib.models import db, Human, Game, Level
...
def skater_to_skater():
    ...
    if not human_id_1 or not human_id_2:
        return render_template('skater_to_skater.html', error="Please provide both human_id_1 and human_id_2 parameters")
    ...
    if not human1 or not human2:
        return render_template('skater_to_skater.html', error="One or both skaters not found")
    
    # Get head-to-head stats
    # SkaterToSkaterStats always stores with skater1_id < skater2_id
    if human_id_1 < human_id_2:
        s2s_stats = db.session.query(SkaterToSkaterStats).filter(
    ...
    if not s2s_stats:
        ...
        return render_template('skater_to_skater.html', 
                              human1=human1, 
                              human2=human2,
    ...
    if stats['games_against'] > 0:
        stats['skater1_goals_per_game'] = stats['skater1_goals'] / stats['games_against']
        stats['skater2_goals_per_game'] = stats['skater2_goals'] / stats['games_against']
        stats['skater1_assists_per_game'] = stats['skater1_assists'] / stats['games_against']
        stats['skater2_assists_per_game'] = stats['skater2_assists'] / stats['games_against']
        stats['skater1_points_per_game'] = stats['skater1_points'] / stats['games_against']
        stats['skater2_points_per_game'] = stats['skater2_points'] / stats['games_against']
        stats['skater1_penalties_per_game'] = stats['skater1_penalties'] / stats['games_against']
        stats['skater2_penalties_per_game'] = stats['skater2_penalties'] / stats['games_against']
    else:
...
def get_level_comparisons(human_id_1, human_id_2):
    ...
    skater1_levels = db.session.query(LevelStatsSkater).filter(
        LevelStatsSkater.human_id == human_id_1
    ).all()
    
    skater2_levels = db.session.query(LevelStatsSkater).filter(
        LevelStatsSkater.human_id == human_id_2
    ).all()
    ...
    for level_id in overlapping_level_ids:
        skater1_stats = next((s for s in skater1_levels if s.level_id == level_id), None)
        skater2_stats = next((s for s in skater2_levels if s.level_id == level_id), None)
