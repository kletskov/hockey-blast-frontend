...
def filter_levels(org_id, human_id=None):
    if org_id == ALL_ORGS_ID:
        ...
        if human_id:
            ...
            org_stats = (
                db.session.query(OrgStatsScorekeeper)
        ...
        if human_id:
            ...
            if org_stats:
                levels = (
                    ...
                    .join(Division, Level.id == Division.level_id)
    ...
    return jsonify(
        [{"id": level.id, "level_name": level.level_name} for level in levels]
    )
...
def get_divisions_and_seasons(org_id, level_id=None, human_id=None):
    query = db.session.query(Division, Season).join(
    ...
    seasons = [
        {"id": season.id, "season_name": season.season_name} for div, season in results
    ]
