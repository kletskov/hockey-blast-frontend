...
def filter_levels(org_id, human_id=None):
    if org_id == ALL_ORGS_ID:
        # For ALL_ORGS_ID, get all levels where this human has scorekeeper stats
        if human_id:
            levels = db.session.query(Level).join(Division, Level.id == Division.level_id).join(
                DivisionStatsScorekeeper, Division.id == DivisionStatsScorekeeper.division_id
            ).filter(DivisionStatsScorekeeper.human_id == human_id).distinct().all()
        else:
        ...
        if human_id:
            levels = db.session.query(Level).join(Division, Level.id == Division.level_id).join(
                ...
                DivisionStatsScorekeeper.human_id == human_id
            ).distinct().all()
        else:
...
def filter_seasons(org_id, level_id, human_id=None):
    ...
    if human_id:
        query = query.join(DivisionStatsScorekeeper, Division.id == DivisionStatsScorekeeper.division_id).filter(
            DivisionStatsScorekeeper.human_id == human_id
        )
